...
function readinessDatabaseUrl(): string {
  const isolated = (process.env.READINESS_DATABASE_URL || '').trim();
  if (isolated) return isolated;

  // A readiness-only deployment must never fall back into the SeenRelay core database.
  if (process.env.SEENRELAY_DEPLOYMENT_ROLE === 'readiness') {
    throw new Error('READINESS_DATABASE_URL is not configured for the readiness-only deployment');
  }

  // Compatibility bridge for the existing main deployment. Remove only after READINESS_DATABASE_URL
  // is configured there as well; this preserves the already-live v1 audit during the cutover.
...
function sql() {
  return neon(readinessDatabaseUrl());
}
