...
export function serviceDescriptor(origin: string) {
  ...
  return {
    ...
    purpose: 'Help agents decide when a known external state needs fresh authoritative validation, using local/private reuse, source-native controls and optional shared recent evidence.',
    semantics: 'Reports what agents recently observed so callers can decide whether to validate again; it does not decide truth or replace the authoritative source.',
    ...
    economics: {
      ...
      fleet_value: 'Local/private/source-native controls should win first. Shared recent evidence is useful only when it can reduce otherwise necessary revalidation under the caller\'s freshness policy.',
