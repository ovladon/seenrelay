...
export function trustDescriptor(origin: string) {
  return {
    ...
    verification: {
      ...
      supplementary_security_analysis: 'CodeQL runs as additional public security evidence. Verify current required checks from the repository ruleset rather than inferring them from workflow presence.',
...
<main>
<section class="hero compact"><div class="eyebrow">VERIFY BEFORE YOU TRUST</div><h1>Do not take SeenRelay on faith.</h1><p class="lead">A freshness preflight sits close to validation, provider spend and agent decisions. That deserves scrutiny. SeenRelay is designed so you can begin with almost no trust: inspect the client source, install a zero-runtime-dependency package, run in shadow mode, keep your original validation path, and verify the live deployment and release evidence yourself.</p><div class="cta"><a class="primary" href="https://github.com/ovladon/seenrelay">Inspect the source</a><a class="secondary" href="/trust.json">Machine-readable posture</a><a class="secondary" href="/data-practices">Data practices</a></div></section>
<section class="section decision"><div class="section-head"><div><div class="eyebrow">MINIMAL BLAST RADIUS</div><h2>Start with a wrapper that is easy to remove.</h2></div><p>Trust should be earned after integration, not required before it.</p></div><div class="proof-grid"><article><b>Zero runtime dependencies</b><span>The public JavaScript/TypeScript and Python client libraries do not pull a dependency tree into your application.</span></article><article><b>Shadow mode first</b><span>CHECK can run while every original validation still executes. No reuse is required until your own evidence and policy justify it.</span></article><article><b>Fail open</b><span>If SeenRelay is unavailable, the wrapper falls back to the validation your application already intended to perform.</span></article><article><b>No lock-in path</b><span>Remove the preflight and keep the original validation function. Leaving does not require exporting SeenRelay-owned application state.</span></article></div></section>
<section class="section split"><div><div class="eyebrow">SOFTWARE SUPPLY CHAIN</div><h2>Verify what you install and how it was released.</h2><p>The client source is public and MIT-licensed. Automated package publishing is configured for registry Trusted Publishing/OIDC instead of long-lived publication tokens. Registry provenance or attestations should be checked for the exact version you install; a release process is evidence, not a substitute for reviewing the package.</p><p><b>Current client:</b> v${esc(trust.client.version)} · ${esc(trust.client.runtime_dependencies)} runtime dependencies.</p></div><div class="terminal"><pre>${esc(trust.client.npm)}
...
<section class="section decision"><div class="section-head"><div><div class="eyebrow">RELEASE BOUNDARY</div><h2>Production should be reproducible from enforced evidence.</h2></div><p>${esc(trust.verification.release_model)}</p></div><div class="proof-grid"><article><b>Required merge gates</b><span>The active main-branch ruleset requires a pull request, the <code>verify</code> check and the isolated <code>preview-release-gate</code>. Inspect the linked ruleset instead of trusting this sentence.</span></article><article><b>Security analysis</b><span>CodeQL runs as additional public evidence. Workflow presence is not represented as merge enforcement unless the repository ruleset also requires it.</span></article><article><b>Live commit</b><span><code>/healthz</code> exposes the Production deployment SHA so an integrator can compare what is live with the public repository.</span></article><article><b>Security reporting</b><span>Security-sensitive reports use the private vulnerability path described in <a href="https://github.com/ovladon/seenrelay/blob/main/SECURITY.md">SECURITY.md</a>, not public exploit disclosure.</span></article></div></section>
<section class="section split"><div><div class="eyebrow">SERVICE BOUNDARY</div><h2>SeenRelay reports observations. It does not ask you to outsource truth.</h2><p>The service does not browse the fact source or use an LLM to decide truth. <b>SAME_OBSERVED</b> is evidence that matching observations exist within the requested window; whether that evidence is reusable remains your policy decision.</p><p>Billing is disabled in the current deployment. Current public access is free and requires no account. The current client cannot silently create a charge on your behalf.</p></div><div class="proof-grid"><article><b>Exactly two operations</b><span>CHECK and OBSERVE.</span></article><article><b>No source credentials</b><span>Credential-bearing and signature-bearing source URLs are rejected from shared fact identity.</span></article><article><b>Bounded evidence</b><span>Observer proofs establish key possession/continuity, not truth or one-key-one-organization identity.</span></article><article><b>Emergency controls</b><span>Runtime modes can reduce capacity, stop new observations, or freeze CHECK/OBSERVE at the application layer.</span></article></div></section>
<section class="section decision"><div class="section-head"><div><div class="eyebrow">WHAT WE DO NOT CLAIM</div><h2>Transparency includes the missing assurances.</h2></div></div><div class="trust-note"><b>No third-party security certification is claimed.</b> SeenRelay currently exposes self-attested, externally inspectable technical controls and public release evidence. A clean automated scan or provenance record does not prove that software is vulnerability-free. Evaluate it under your own risk model and begin in shadow mode.</div></section>
<section class="section final"><div><div class="eyebrow">VERIFY IT YOURSELF</div><h2>Inspect first. Measure in shadow mode. Reuse only when your policy accepts the evidence.</h2></div><div class="cta"><a class="primary" href="/quickstart">Run a shadow pilot</a><a class="secondary" href="https://github.com/ovladon/seenrelay/blob/main/SECURITY.md">Security policy</a><a class="secondary" href="https://github.com/ovladon/seenrelay/blob/main/docs/THREAT_MODEL.md">Threat model</a></div></section>
