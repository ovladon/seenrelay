...
export function publicLandingPage(origin: string): string {
  ...
  const claudeMarketplaceAdd = esc(f.install.claude_marketplace_add_command);
  const claudePluginInstall = esc(f.install.claude_plugin_install_command);
    ...
    <div class="rv-console rv-funnel-console" aria-live="polite">
      ...
      <div class="rv-console-body">
        <div class="rv-install-view active" data-install-view="agent" id="agent-audit">
          ...
          <div class="rv-step"><span>2</span><div><h4>Install for your coding agent</h4><p>Claude Code native plugin:</p><div class="rv-code"><pre id="claude-plugin-install">${claudeMarketplaceAdd}\n${claudePluginInstall}</pre><button class="rv-copy" type="button" data-copy-target="claude-plugin-install">Copy</button></div><p>Other Agent Skills clients:</p><div class="rv-code"><pre id="skill-install">${esc(skillCommand)}</pre><button class="rv-copy" type="button" data-copy-target="skill-install">Copy</button></div></div></div>
