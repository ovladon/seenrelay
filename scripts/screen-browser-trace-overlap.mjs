...
export function screenBrowserOverlap(parsedSessions, {
  ...
  for (const call of eligible) {
    ...
    if ([...priorExactSessions].some((session) => session !== call.session)) crossSessionExactReuseOpportunities += 1;
    ...
    if ([...priorUrlSessions].some((session) => session !== call.session)) crossSessionUrlReuseOpportunities += 1;
...
function main() {
  ...
  const report = screenDirectory(args.inputDir, {
    ...
    sourceDataset: args.sourceDataset || 'unspecified'
  });
