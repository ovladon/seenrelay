...
async function req(base,path,init={}){return fetch(`${base}${path}`,{...init,headers:{...(init.headers||{}),'x-vercel-protection-bypass':BYPASS},cache:'no-store',redirect:'manual',signal:AbortSignal.timeout(15000)});}
...
async function measure(){
  ...
  console.log(JSON.stringify({schema:'seenrelay-smoke-fast-measure-v1',semantic_parity:true,trials:samples.length,first_trial:samples[0],steady_app_mean_ms:mean(steady.map(x=>x.app_ms)),steady_app_median_ms:median(steady.map(x=>x.app_ms)),steady_cpu_mean_ms:mean(steady.map(x=>x.cpu_ms)),steady_cpu_median_ms:median(steady.map(x=>x.cpu_ms)),samples,raw_lease_emitted:false},null,2));
}
