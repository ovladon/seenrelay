...
for(let i=0;i<N;i++){
  ...
  records.push({trial_id:`candidate-v3-${ENV_LABEL}-${String(i+1).padStart(3,'0')}`,http_status:r.status,assurance:{status:b.status,known_value_hash_equals_latest_value_hash:true,observer_keys:Number(b.recent_observer_keys),cryptographic_observer_keys:Number(b.recent_cryptographic_observer_keys),reuse_independence_buckets:Number(b.recent_reuse_independence_buckets),age_seconds:Number(b.age_seconds),contested:false,source_validator_present:true,server_timing_app_present:true,server_timing_cpu_present:true},accounting:{relay_measured:true,all_network_legs_included:true,authoritative_source_requests:0,authoritative_source_request_basis:'CHECK implementation is source-non-browsing and bound by the admitted source commit',platform_overhead_status:'UNKNOWN_NOT_ZERO',raw_lease_emitted:false,raw_response_body_retained:false,semantic_parse_cpu_included:false,agent_cpu_scope:'request_through_receive_accumulation_matching_native_collector',agent_cpu_zero_delta_is_valid_measurement:true},metrics:r.metrics});
}
const aggregateAgentCpu=records.reduce((s,r)=>s+r.metrics.agent_cpu_ms,0)/records.length;assert.ok(aggregateAgentCpu>0,'aggregate agent CPU signal missing');
const core={schema:'seenrelay-preview-action-candidate-sample-v2',contract_fingerprint:CONTRACT_FP,target_preview_sha:TARGET_SHA,fact_coordinate_fingerprint:FACT_FP,sample_type:'controlled_real_service',caller_environment:ENV_LABEL,github_run_id:String(RUN_ID),trials:N,no_interim_selection:true,no_record_replacement:true,first_measured_check_includes_lease_establishment_if_needed:true,platform_overhead_status:'UNKNOWN_NOT_ZERO',agent_cpu_resolution_policy:'individual_zero_allowed_aggregate_positive_required',raw_lease_emitted:false,raw_response_bodies_retained:false,records};
const out={...core,evidence_fingerprint:sha256(core)};writeFileSync(OUT,`${JSON.stringify(out,null,2)}\n`,{flag:'wx'});console.log(JSON.stringify({schema:out.schema,caller_environment:ENV_LABEL,target_preview_sha:TARGET_SHA,trials:N,agent_cpu_zero_count:records.filter(r=>r.metrics.agent_cpu_ms===0).length,aggregate_agent_cpu_mean_ms:aggregateAgentCpu,evidence_fingerprint:out.evidence_fingerprint,raw_lease_emitted:false,production_authorized:false,economic_claim_authorized:false},null,2));
