...
import { firecrawlResultFingerprint, firecrawlScrapePolicy } from './firecrawl.js';
import { evaluateHostileBenchmark } from './economics.js';
...
function providerCredits(result) {
  const credits = Number(parseFirecrawlMcpResult(result)?.metadata?.creditsUsed);
...
/**
 * Natural-workload Firecrawl measurement. The authoritative provider call always runs and returns
 * before counterfactual CHECK/OBSERVE work is awaited by the caller.
 */
export class FirecrawlShadowPilot {
  ...
  async callTool(params, ...rest) {
    ...
    // CHECK starts after the authoritative provider call and before the current observation is
    // deposited, so shadow measurement cannot suppress validation or manufacture its own hit.
  ...
  hostileBenchmarkInput(options = {}) {
    ...
    return Object.freeze({
      ...
      controls: {
        ...
        // Every provider call still runs, so provider-native cache behavior is included in the
        // measured Firecrawl baseline instead of being disabled for a favorable comparison.
